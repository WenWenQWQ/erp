...
export default class BuyStorage extends React.Component{
    ...
    //编辑保存表格
    handleSave = (row) => {
        row.amount=parseFloat(row.price*row.number).toFixed(2);
        row['amount_rate']=parseFloat(row.amount*row.rate+parseFloat(row.amount)).toFixed(2);
    ...
    request=(_id)=>{
        if(_id&&_id!==''){
            fetch('/purchasing/product/detail',{
                method:'POST',
                headers:{
    ...
    handleSubmit=()=>{
        ...
        orderHeader.validateFields((err,value)=>{
            if(!err) {
                let storageDate = moment(value.storageDate).format("YYYY-MM-DD");
                const orderHeader = {
                    ...value,
                    storageDate
                };
                ...
                orderFooter.validateFields((ferr,fvalue)=> {
                    if(!ferr){
                        ...
                        fetch('/purchasing/storage',{
                            ...
                            credentials:'include',
                            mode:"cors",
                            body:JSON.stringify(Object.assign(order,{IRmark:'采购入库'}))
                        }).then(
                            res=>res.json()
                        ).then(data=>{
                            ...
                            this.setState({
                                ...
                                proInfos.forEach((item)=>{
                                    ...
                                    fetch('/houseDetail',{
                                        ...
                                        credentials:'include',
                                        mode:"cors",
                                        body:JSON.stringify(Object.assign(proInfo,{IRmark:'采购入库'}))
                                    });
                                    fetch('/stock',{
                                        method:'POST',
                                        headers:{
                                   ...
                                   /* console.log(proInfo);
                                    console.log(proInfo.purHouseId)*/
                                    fetch('/purchasing/order/update',{
                                        method:'POST',
                                        headers:{
                                            'Accept':'application/json',
                                            'Content-Type':'application/json'
                                        ...
                                        credentials:'include',
                                        mode:"cors",
                                        body:JSON.stringify({_id:proInfo._id})
                                    })
                                })
                            })
                        }).catch(function (err) {
        ...
        return(
            <Row className="order">
                <Row className="header">
                    <Col span={12} className="title">采购入库</Col>
           ...
           <Row>
               <Col className="date">
                   <Form {...formItemLayout} autoComplete="off" layout="inline">
                       ...
                       <Form.Item label="采购订单编号">
                           {
                               getFieldDecorator("orderId",{
                                   rules:[{
                                       required:true,message:'请选择采购订单编号'
                                   }]
                               })(
                       ...
                       <Form.Item label="入库人员">
                           {
                               getFieldDecorator("storager",{
                                   rules:[{
